...
对每帧扫一组 K=(4,6,8,10), 每帧独立聚类, 输出:
  ...
  3) kmeans_aerial_contact_sheet_k<K>.png  某个 K 下所有帧的汇总
...
SESS = f"{ROOT}/workspaces/session_20260531_023910_280850"
OUT  = f"{ROOT}/experiments/dinov2_test"
os.makedirs(f"{OUT}/seg_aerial", exist_ok=True)
os.makedirs(f"{OUT}/seg_grid_aerial", exist_ok=True)
...
for p in imgs:
    ...
    for k in KS:
        ...
        cv2.imwrite(f"{OUT}/seg_aerial/{idx:06d}_k{k}.png", side)
        by_k[k].append(side)
        # 给 grid 用: 分割图上标 K
        tag = blend.copy()
    ...
    cv2.imwrite(f"{OUT}/seg_grid_aerial/{idx:06d}.png", row)
...
for k in KS:
    ...
    cv2.imwrite(f"{OUT}/kmeans_aerial_contact_sheet_k{k}.png", sheet)
    print(f"wrote kmeans_aerial_contact_sheet_k{k}.png")
print("done")
