"""Gridded DEM elevation figure for scale_test (20260804).

Unlike ``elevation_3d_views.png`` (a smooth-shaded H_top surface without visible
cells), this reproduces the *gridded* DEM that the main service builds in
...
Two panels come from the two documented DEM producers in the main pipeline:
  * terrain DEM  — build_elevation_view_grid, objects removed (viewer path)
  * surface-top DEM — terrain_analysis.rasterize_bev H_top (export dem_source='htop'),
    objects kept, so the three boxes rise out of the grid.
"""
...
OBJECT_IDS = (3, 4, 5)          # the three boxes: excavator-equivalent objects kept off the terrain


def build_metric_dem():
    z = np.load(WS / "predictions.npz")
    pred = {k: np.asarray(z[k]) for k in z.files}
    results = json.loads((WS / "experiment_results.json").read_text(encoding="utf-8"))
    ...
    #     This one KEEPS objects, so the three boxes stand out of the grid.
...
def render(dem: dict) -> Path:
    ...
    surf = _draw_surface(ax2, xx, zz, htop_grid, dem, vmin, vmax,
                         ...
                         f"objects kept — three boxes rise from the grid")
